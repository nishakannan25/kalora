...
class PotteryTerracottaPotsClassifier:
    ...
    def predict(self, image_input):
        ...
        with torch.no_grad():
            ...
            confidence = float(probs[pred_idx]) if len(self.classes) > 1 else 1.0
        ...
        return {
            ...
            "class_probabilities": {self.idx_to_class[i]: float(probs[i]) if len(self.classes) > 1 else 1.0 for i in range(len(self.classes))}
        }

if __name__ == "__main__":
    classifier = PotteryTerracottaPotsClassifier()
    print("PotteryTerracottaPotsClassifier initialized successfully.")
    print(f"Loaded {len(classifier.classes)} terracotta classes: {classifier.classes}")
