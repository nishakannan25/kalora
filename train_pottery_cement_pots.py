...
POTTERY_DIR = os.path.join(BASE_DIR, "05_Cement_Pots")
IMAGES_DIR = os.path.join(POTTERY_DIR, "images")
META_CSV = os.path.join(POTTERY_DIR, "metadata.csv")

MODEL_DIR = os.path.join(BASE_DIR, "models", "pottery_cement_pots")
...
def derive_cement_category(row):
    ...
    if any(w in full_text for w in ["planter", "pot", "planters", "pots", "trough", "urn"]):
        return "Cement Planter & Pot"
    elif any(w in full_text for w in ["vase", "vases"]):
        return "Cement Decorative Vase"
    elif any(w in full_text for w in ["statue", "sculpture", "figurine", "decor"]):
        return "Cement Statue & Art"
    else:
        return "General Cement Craft"

df_vision["category_label"] = df_vision.apply(derive_cement_category, axis=1)
...
# -------------------------------------------------------------
class CementPotDataset(Dataset):
...
ds_tr = CementPotDataset(df_tr, transform=train_tf)
ds_val = CementPotDataset(df_val, transform=eval_tf)
ds_te = CementPotDataset(df_te, transform=eval_tf)
...
model_config = {
    "model_name": "PotteryCementPotsClassifier",
    "dataset_name": "05_Cement_Pots",
