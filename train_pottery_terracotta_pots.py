...
POTTERY_DIR = os.path.join(BASE_DIR, "02_Terracotta_Pots")
IMAGES_DIR = os.path.join(POTTERY_DIR, "images")
META_CSV = os.path.join(POTTERY_DIR, "metadata.csv")

MODEL_DIR = os.path.join(BASE_DIR, "models", "pottery_terracotta_pots")
...
def derive_terracotta_category(row):
    ...
    if any(w in full_text for w in ["diya", "lamp", "diyas", "deepak"]):
        return "Terracotta Diya & Lamp"
    elif any(w in full_text for w in ["planter", "pot", "planters", "pots"]):
        return "Terracotta Planter & Pot"
    elif any(w in full_text for w in ["vase", "vases"]):
        return "Terracotta Decorative Vase"
    elif any(w in full_text for w in ["mug", "cup", "bowl", "plate", "jar", "crockery", "dinnerware", "bottle", "jug", "kulhad"]):
        return "Terracotta Dining & Kitchenware"
    elif any(w in full_text for w in ["statue", "idol", "figurine", "showpiece", "decor", "sculpture", "wall hanging"]):
        return "Terracotta Art & Home Decor"
    else:
        return "General Terracotta Craft"

df_vision["category_label"] = df_vision.apply(derive_terracotta_category, axis=1)
...
# -------------------------------------------------------------
class TerracottaPotDataset(Dataset):
...
ds_tr = TerracottaPotDataset(df_tr, transform=train_tf)
ds_val = TerracottaPotDataset(df_val, transform=eval_tf)
ds_te = TerracottaPotDataset(df_te, transform=eval_tf)
...
model_config = {
    "model_name": "PotteryTerracottaPotsClassifier",
    "dataset_name": "02_Terracotta_Pots",
