...
POTTERY_DIR = os.path.join(BASE_DIR, "04_Clay_Pots")
IMAGES_DIR = os.path.join(POTTERY_DIR, "images")
META_CSV = os.path.join(POTTERY_DIR, "metadata.csv")

MODEL_DIR = os.path.join(BASE_DIR, "models", "pottery_clay_pots")
...
def derive_clay_category(row):
    ...
    if any(w in full_text for w in ["diya", "lamp", "diyas", "deepak"]):
        return "Clay Diya & Lamp"
    elif any(w in full_text for w in ["planter", "pot", "planters", "pots"]):
        return "Clay Planter & Pot"
    elif any(w in full_text for w in ["vase", "vases"]):
        return "Clay Decorative Vase"
    elif any(w in full_text for w in ["tawa", "cooker", "handi", "jug", "bottle", "kadai", "bowl", "cup", "kulhad"]):
        return "Clay Cooking & Diningware"
    elif any(w in full_text for w in ["statue", "idol", "figurine", "showpiece", "decor", "sculpture"]):
        return "Clay Art & Home Decor"
    else:
        return "General Clay Craft"

df_vision["category_label"] = df_vision.apply(derive_clay_category, axis=1)
...
ds_tr = ClayPotDataset(df_tr, transform=train_tf)
ds_val = ClayPotDataset(df_val, transform=eval_tf)
ds_te = ClayPotDataset(df_te, transform=eval_tf)
...
model_config = {
    "model_name": "PotteryClayPotsClassifier",
    "dataset_name": "04_Clay_Pots",
